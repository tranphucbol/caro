...
class Room extends React.Component {
    ...
    componentDidMount() {
        document.querySelectorAll('.db-room').forEach(room => room.addEventListener('mouseenter', () => {
            let img = room.querySelector('.db-room-point img')
            img.src = `${process.env.PUBLIC_URL}/images/coin.gif`
        }))

        document.querySelectorAll('.db-room').forEach(room => room.addEventListener('mouseleave', () => {
            let img = room.querySelector('.db-room-point img')
